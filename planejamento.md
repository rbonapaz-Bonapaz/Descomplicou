...
### C.2 Cartão, parcelamento e link de pagamento (Grupo C antigo)
**Solução**:
...
**H.12 Evolução do link de eventos** — ✅ CONCLUÍDO (15/07/2026, v99)
Toggle "Mostrar todo o catálogo Farmasi no link" — sem marcar, só linhas participantes aparecem (comportamento antigo); marcando, todo produto ativo aparece, mas o desconto continua restrito às linhas escolhidas (demais pelo preço normal). Snapshot de cada produto ganhou `prontaEntrega` (estoque no momento da publicação); vitrine pública mostra tag verde "X em pronta entrega" ou cinza "Sob encomenda".
**Arquivos**: `js/eventos.js`, `js/evento-publico.js`, `evento.html` (CSS)
**Verificação**: toggle presente no formulário; fluxo de criação passa da validação e chega à escrita real (bloqueada só por permissão, esperado).
...
**H.17 Correção automática de valores por IPCA** — 🆕: reajuste anual automático dos planos, com aviso prévio ao cliente — precisa de fonte de dados do IPCA (API externa) e regra de notificação; maior escopo, vale conversa de design antes
**H.18 Promoção de indicação (admin)** — 🆕: toggle no admin; indicação de nova consultora rende 1 mês grátis automático
...
Conteúdo da aba "Sobre" (manuais com GIFs, vídeos de divulgação) e funcionalidades estratégicas futuras: alertas de reposição por ciclo de vida do produto, controle de validade/vencimento no estoque, tags de clientes, "dinheiro parado" (curva ABC), assistente de IA pro WhatsApp (expansão do Gemini já configurado), relatórios avançados com cards configuráveis. Todos fazem sentido só depois que a Fase H (uso diário) estiver estável — não há decisão de arquitetura pendente ainda, só volume de trabalho.
...
1. **Deploy do `firestore.rules`** — as regras de multi-admin/base coletiva estão escritas mas nunca publicadas. Rodar `firebase deploy --only firestore:rules` ou colar no Console. Sem isso, telas de admin/config de planos falham por permissão — e a Fase F depende disso também
...
| 4 | B.4 Frete | Médio | 🔨 Implementado (sintaxe OK; testar com dados reais) |
