...
function pageHtml(items, title, density) {
  ...
  return `<section class="cat-page ${density}">
    <header class="cat-header">
      ...
      <div class="cat-consult">${esc(p.nomeNegocio || 'FaroBella')}<br>${esc(p.nome || '')}${p.whatsapp ? `<br>📱 ${esc(p.whatsapp)}` : ''}${p.instagram ? `<br>📷 @${esc(String(p.instagram).replace(/^@/, ''))}` : ''}</div>
    </header>
    ...
    <footer class="cat-footer">
      ...
      <div class="cat-foot-text"><b>${esc(p.nomeNegocio || 'FaroBella')}</b><br>${esc(p.rodapeCatalogo ?? 'Fale comigo para fazer seu pedido')}<br>${esc(p.linkLoja || qrData())}</div>
